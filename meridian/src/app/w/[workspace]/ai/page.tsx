...
export default function AiPage() {
  ...
  async function send(text: string) {
    ...
    try {
      ...
      const turn = await askCto(workspaceId, prompt, history, { pathname: `/w/${workspaceId}/ai` });
      pushAi({
        role: "assistant",
        body: turn.reply,
        model: turn.model,
      ...
      setMeta(`${turn.model} · ${turn.toolCalls.map((call) => call.name).join(", ") || "no tools"} · ${turn.grounded.join(" · ") || "no citations"}`);
    } catch (error) {
      pushAi({
        role: "assistant",
