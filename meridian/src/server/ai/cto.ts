...
export function llmConfig(): LlmConfig | null {
  ...
  if (!resolved.configured || !resolved.baseUrl) return null;
...
function planLocally(prompt: string): Array<{ name: string; args: Record<string, string> }> {
  ...
  if (/improve|fix (the )?(platform|app)|what should we|auto/.test(lower)) {
...
function summarize(prompt: string, calls: CtoTurn["toolCalls"], grounded: string[]): string {
  ...
  for (const call of calls) {
    ...
    if ((call.name === "auto_improve" || call.name === "list_improvements") && result && typeof result === "object") {
...
export async function runCto(
  ...
  const rag = queryRag(workspaceId, prompt, 4);
